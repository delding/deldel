    ...
    public List<int[]> getSkyline(int[][] buildings) {
        ...
        Arrays.sort(buildings, new Comparator<int[]>(){
            public int compare(int[] b1, int[] b2) {
                if (b1[0] == b2[0]) return b2[2] - b1[2]; // ERROR: if left point is equal must make the highest building comes first
        ...
        for (int i = 0; i < buildings.length; i++) {
            ...
            while (skylineR < b[0] && !maxq.isEmpty()) { // all heights in maxq are <= skylineH
                while (!maxq.isEmpty() && maxq.peek()[1] <= skylineR) maxq.poll(); // ERROR: take out useless building
