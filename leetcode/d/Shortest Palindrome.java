    ...
    private int kmp(String txt, String pattern) {
        ...
        while (i <= txt.length() - len) {
            if (txt.charAt(i) == pattern.charAt(j)) {
                i++;
                j++;
                if (j == len) return i - j; // to find next match, set j = lps[len - 1] i.e. the lps of entire pattern
            } else {
                if (j == 0) i++;
                else j = lps[j-1]; // length of lps of left matching part is also the next j position, because the part before lps[j-1] is the matching prefix of left part's suffix, lps[j-1] is the next position that needs to be matched
            }
        }
        return -1;
    }
