...
public class Solution {
    ...
    private void dfs1(List<Integer> rst, String input) {
        if (input.length() == 1) rst.add(Integer.parseInt(input)); // ERROR: len of number can be larger than 1, also number can be minus
    ...
    private int compute(int i, String input) {
        ...
        switch (op) {
            ...
            case '-': rst = num1 - num2; break;
            case '*': rst = num1 * num2; break;
            default: rst = Integer.MIN_VALUE; // invalid op
        }
        return rst;
    }
}
