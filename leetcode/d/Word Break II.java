...
public class Solution {
    ...
    public List<String> wordBreak(String s, Set<String> wordDict) {
        ...
        for (int i = 0; i < s.length(); i++) {
            if (i > 0 && memo.get(i - 1).isEmpty()) continue; // ERROR: TLE without this optimazion, skip curr index if no words end at previous index
    ...
    private void dfs(List<Set<String>> memo, List<String> ret, String sentence, int idx) {
        ...
        if (memo.get(idx).isEmpty()) return; // ERROR: words end at index idx can be zero, need to check first
